!(function (t) {
	...
	function n(e, i) {
		...
		return (
			...
			!this.settings.setTitle &&
				e.data("chocolat-title") &&
				(this.settings.setTitle = e.data("chocolat-title")),
	...
	t.extend(n.prototype, {
		...
		preload: function (e) {
			...
			if ("undefined" != typeof this.settings.images[e]) {
				...
				return (
					...
					(s.src = this.settings.images[e].src),
					i
				);
			}
		},
			...
			if (
				...
				this.elems.overlay.fadeIn(this.settings.duration),
					...
					(this.settings.timer = setTimeout(function () {
						"undefined" != typeof i.elems &&
							t.proxy(i.elems.loader.fadeIn(), i);
					}, this.settings.duration));
				var s = this.preload(e)
						.then(function (t) {
							return i.place(e, t);
						})
						.then(function (t) {
					...
					n = e + 1;
				return (
					"undefined" != typeof this.settings.images[n] && this.preload(n), s
				);
			}
		},
		place: function (t, e) {
			...
			return (
				...
				this.pagination(),
				this.arrows(),
				this.storeImgSize(e, t),
				(i = this.fit(t, s.elems.wrapper)),
				this.center(i.width, i.height, i.left, i.top, 0)
			);
		},
		center: function (t, e, i, s, n) {
			return this.elems.content
				.css("overflow", "visible")
				.animate({ width: t, height: e, left: i, top: s }, n)
				.promise();
		},
		appear: function (t) {
			var e = this;
		...
		fit: function (e, i) {
			...
			return (
				"cover" == this.settings.imageSize
					...
					? ((s = o), (n = a))
					: (f > u ? ((s = g), (n = s / f)) : ((n = m), (s = n * f)),
					  "default" === this.settings.imageSize &&
							(n >= a || s >= o) &&
							((n = a), (s = o))),
				{ height: s, width: n, top: (l - s) / 2, left: (r - n) / 2 }
			);
		},
		change: function (t) {
		...
		arrows: function () {
			this.settings.loop
				? t([this.elems.left[0], this.elems.right[0]]).addClass("active")
				...
				: t([this.elems.left[0], this.elems.right[0]]).removeClass("active");
		},
		description: function () {
			var t = this;
			this.elems.description.html(
				t.settings.images[t.settings.currentImage].title
			);
		},
		pagination: function () {
			var t = this,
		...
		storeImgSize: function (t, e) {
			"undefined" != typeof t &&
				((this.settings.images[e].height && this.settings.images[e].width) ||
					((this.settings.images[e].height = t.height),
					(this.settings.images[e].width = t.width)));
		},
		close: function () {
		...
		markup: function () {
			this.elems.domContainer.addClass(
				...
				(this.elems.wrapper = t("<div/>", {
					class: "chocolat-wrapper",
					id: "chocolat-content-" + this.settings.setIndex,
				}).appendTo(this.elems.domContainer)),
				(this.elems.overlay = t("<div/>", {
					class: "chocolat-overlay",
				}).appendTo(this.elems.wrapper)),
				(this.elems.loader = t("<div/>", { class: "chocolat-loader" }).appendTo(
					this.elems.wrapper
				)),
				(this.elems.content = t("<div/>", {
					class: "chocolat-content",
				}).appendTo(this.elems.wrapper)),
				(this.elems.img = t("<img/>", {
					class: "chocolat-img",
					src: "",
				}).appendTo(this.elems.content)),
				(this.elems.top = t("<div/>", { class: "chocolat-top" }).appendTo(
					this.elems.wrapper
				)),
				(this.elems.left = t("<div/>", { class: "chocolat-left" }).appendTo(
					this.elems.wrapper
				)),
				(this.elems.right = t("<div/>", { class: "chocolat-right" }).appendTo(
					this.elems.wrapper
				)),
				(this.elems.bottom = t("<div/>", { class: "chocolat-bottom" }).appendTo(
					this.elems.wrapper
				)),
				(this.elems.close = t("<span/>", { class: "chocolat-close" }).appendTo(
					this.elems.top
				)),
				(this.elems.fullscreen = t("<span/>", {
					class: "chocolat-fullscreen",
				}).appendTo(this.elems.bottom)),
				(this.elems.description = t("<span/>", {
					class: "chocolat-description",
				}).appendTo(this.elems.bottom)),
				(this.elems.pagination = t("<span/>", {
					class: "chocolat-pagination",
				}).appendTo(this.elems.bottom)),
				(this.elems.setTitle = t("<span/>", {
		...
		openFullScreen: function () {
			...
			t.requestFullscreen
				...
				? ((this.settings.fullscreenOpen = !0), t.mozRequestFullScreen())
				: t.webkitRequestFullscreen
				? ((this.settings.fullscreenOpen = !0), t.webkitRequestFullscreen())
				: t.msRequestFullscreen
				? (t.msRequestFullscreen(), (this.settings.fullscreenOpen = !0))
				: (this.settings.fullscreenOpen = !1);
		},
		exitFullScreen: function () {
			i.exitFullscreen
				? (i.exitFullscreen(), (this.settings.fullscreenOpen = !1))
				: i.mozCancelFullScreen
				? (i.mozCancelFullScreen(), (this.settings.fullscreenOpen = !1))
				: i.webkitExitFullscreen
				? (i.webkitExitFullscreen(), (this.settings.fullscreenOpen = !1))
				: i.msExitFullscreen
				? (i.msExitFullscreen(), (this.settings.fullscreenOpen = !1))
		...
		events: function () {
			...
			t(i)
				...
				.on("keydown.chocolat", function (t) {
					s.settings.initialized &&
						(37 == t.keyCode
							? s.change(-1)
							: 39 == t.keyCode
							? s.change(1)
				...
				t([this.elems.overlay[0], this.elems.close[0]])
					.off("click.chocolat")
					.on("click.chocolat", function () {
						return s.close();
					}),
				this.elems.fullscreen
					.off("click.chocolat")
					.on("click.chocolat", function () {
						return s.settings.fullscreenOpen
							? void s.exitFullScreen()
							: void s.openFullScreen();
					}),
				s.settings.backgroundClose &&
					this.elems.overlay
						.off("click.chocolat")
						.on("click.chocolat", function () {
							return s.close();
				...
				this.elems.wrapper
					...
					.on("click.chocolat", function (t) {
						if (
							null === s.settings.initialZoomState &&
							s.elems.domContainer.hasClass("chocolat-zoomable")
						)
							return t.stopPropagation(), s.zoomIn(t);
					}),
				this.elems.wrapper.mousemove(function (e) {
					if (
						null !== s.settings.initialZoomState &&
						!s.elems.img.is(":animated")
					) {
						var i = t(this).offset(),
							n = t(this).height(),
							o = t(this).width(),
						...
						var d = { "margin-left": -h + "px", "margin-top": -g + "px" };
						"undefined" != typeof e.duration
							? t(s.elems.img).stop(!1, !0).animate(d, e.duration)
							: t(s.elems.img).stop(!1, !0).css(d);
					}
				}),
				t(e).on("resize", function () {
					s.settings.initialized &&
						s.settings.currentImage !== !1 &&
						s.debounce(50, function () {
		...
		zoomable: function () {
			var t = this.settings.images[this.settings.currentImage],
				...
				n =
					this.elems.img.width() > t.width ||
					this.elems.img.height() > t.height;
			s && !n
		...
		zoomIn: function (e) {
			...
			(i.pageX = e.pageX),
				(i.pageY = e.pageY),
				(i.duration = this.settings.duration),
				this.elems.wrapper.trigger(i),
		...
		zoomOut: function (t, e) {
			if (
				null !== this.settings.initialZoomState &&
				this.settings.currentImage !== !1
			) {
				(e = e || this.settings.duration),
					(this.settings.imageSize = this.settings.initialZoomState),
					(this.settings.initialZoomState = null),
					this.elems.img.animate({ margin: 0 }, e),
		...
		setDomContainer: function () {
			this.settings.container === e
				? (this.elems.domContainer = t("body"))
				: (this.elems.domContainer = t(this.settings.container));
		},
	...
	return (
		(t.fn.Chocolat = function (e) {
			return this.each(function () {
				o++;
				var i = t.extend(!0, {}, a, e, { setIndex: o });

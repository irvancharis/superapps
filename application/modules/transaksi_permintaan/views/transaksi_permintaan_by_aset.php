...
$(document).ready(function() {
    ...
    document.getElementById("KODE_ASET").addEventListener("keyup", function(event) {
        if (event.key === "Enter") {
            $.ajax({
                url: "<?php echo base_url('transaksi_permintaan/get_produk_by_aset/'); ?>",
                type: "POST",
                dataType: "JSON",
                data: {
    ...
    function saveFormData() {
        let formData = {
            ...
            KETERANGAN: $('#KETERANGAN_PERMINTAAN').val() == '' ? null : $('#KETERANGAN_PERMINTAAN')
                .val()
        };
        ...
        storedProdukItems.forEach(function(item, index) {
            tbody.append(`
                                <tr data-index="${index}">
                                    ...
                                    <input type="hidden" class="form-control" name="JUMLAH_PERMINTAAN[${index}]" value="1">
    ...
    function attachInputListeners() {
        $('#selected-items-body').on('input', '.JUMLAH_PERMINTAAN', function() {
            let rowIndex = $(this).closest('tr').data('index');
            let stokReal = $(this).val();

            let storedItems = JSON.parse(localStorage.getItem('storedProdukItems')) || [];
            storedItems[rowIndex].JUMLAH_PERMINTAAN = stokReal;
